...
重力の増大で計算不能になる原因はコンピューターの能力の限界もありますが、
物理法則としての実在性についても疑問に思っています。
無限大が出現する原因は双曲線関数の適用にあるので、ただの三角関数による回転によればその矛盾は回避されると考えました。
なぜ一般相対性理論を頼らないのかについても理由がありますが、長くなりすぎるので別の機会にします。

\subsection{双対時空での重力計算}

これまで重力による影響の結果、双対時空の時空立方体が回転する様子を具体的に三次元で見てきました。
この時空立方体の回転効果がどのように自由落下の見せかけの加速度に変換されるかを考えて計算していきます。
回転した時空立方体に四元数における対数関数と同型の計算を行って、角度を抽出し直します。
要するに、一回転すると元に戻る計算を三次元でしています。
