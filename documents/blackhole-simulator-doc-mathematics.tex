...
一般的に、組み合わせて使用する1-ベクトルの内、2乗して$1$になる1-ベクトルの数を$m$個、
...
\begin{table}[ht]
  ...
  \begin{tabular}{|l|l|l|} \hline
    ...
    グレード対合(Grade Involution) & $GI(A)$ & 奇数グレードの符号反転 \\ \hline
    クリフォード共役(Clifford Conjugate) & $\overline{A}$ & 他の共役の掛け合わせ \\ \hline
  \end{tabular}
\end{table}
...
この回転子を以下の様に対象$M$に作用させます。
\begin{equation}
  RM\widetilde{R}=X'
\end{equation}
この様な挟み込みの掛け合わせで対象を変換することを、サンドウィッチ積(Sandwich Product)と呼びます。
通常、サンドウィッチ積による操作は対象の示す座標を回転させる事になります。
...
1-ベクトルよりなる時空を双ベクトルによるサンドウィッチ積で回転変換する事を考えます。
...
\end{equation}
擬スカラー$e_0e_1e_2e_3$は幾何代数的には4次元時空の体積を表しています。
双対で算出した3-ベクトルは時空の体積と関係していると考えられます。
双対時空と呼ぶ事にします。
通常時空と双対時空の和についても不変量となっている様子です。
...
双対回転子により双対時空をサンドウィッチ積すると、ローレンツブーストになりました。
...
双対サンドウィッチ積後の双対時空$(S^\dag)'=R^\dag S^\dag\widetilde{R}^\dag$は、円運動の移動方向と逆方向の遠心力を表現していると考えられます。
...
なぜy軸だけ符号反転するのかという疑問が沸きます。
正確な表現を使うと、3-ベクトルの係数は座標ではなく体積を表すための長さを表していると考えられます。
例えば、$te_1e_2e_3$の$t$は、三次元空間から時間方向へのどのくらい膨らんでいるかの長さになるでしょう。
その違いが、左手系と右手系の違いのような差になって現れて、y軸が符号反転することで辻褄が合うようになっていると考えられます。
...
$(R^\dag)^\dag$が時空にサンドウィッチ積されると加速になります。
...
\end{equation}
サンドウィッチ積により重力の影響で歪んだ双対時空$ (S^\dag)' = R^\dag S^\dag \widetilde{R^\dag} $を計算します。
...
サンドウィッチ積の順序が逆になっていて回転角度が逆になったと考えられます。
本質的な部分は変わりません。
