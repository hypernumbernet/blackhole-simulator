...
* シミュレーションの操作
  ...
  - "Monitoring..." をクリックしてダイアログを開き、現在の粒子の位置を数値で取得できます。
...
粒子同士がボムボールのようにバウンドしたら面白いと思い衝突の計算を付加したものです。結果として星の誕生のようなシミュレーションになりました。条件によって連星になったりします。
...
そこで、7次元のうちどの4つを使用するかの組み合わせ、Combination(7, 4) の35パターン全てを計算し表示してみています。それが、Grid line の Octonion S3 Rot. になります。
